...
const REGIONES_CHILE = [
  ...
  { nombre: "Biobío", comunas: ["Concepción", "Talcahuano", "Los Ángeles", "San Pedro de la Paz", "Chiguayante", "Coronel"] },
  ...
  { nombre: "Aysén del General Carlos Ibáñez del Campo", comunas: ["Coyhaique", "Aysén"] },
...
function CheckoutPage() {
  ...
  const [formData, setFormData] = useState({
    email: '', nombre: '', apellido: '', direccion: '', region: '', ciudad: '', codigoPostal: '', telefono: '',
  });
  ...
  useEffect(() => {
    ...
    if (!isTokenValid()) { navigate('/login', { replace: true }); return; }
  ...
  const subtotal = cartItems.reduce((acc, item) => acc + (item.producto?.precio || 0) * item.cantidad, 0);
  const envio = calcularEnvio(subtotal);
  const totalPedido = subtotal + envio;
  
  if (loadingAuth || loadingCart) return <div className="checkout-container"><p>Cargando...</p></div>;

  return (
    <main className="checkout-container">
      {cartError ? <div className="checkout-error"><p>{cartError}</p></div> : cartItems.length === 0 ? (
        <div className="checkout-empty"><p>Carrito vacío</p><button onClick={() => navigate('/productos')}>Ver Productos</button></div>
      ) : (
        <div className="checkout-layout">
          <div className="checkout-form-section">
            <h1 className="checkout-title">Finalizar Compra</h1>
            <form onSubmit={handleSubmit} className="checkout-form">
              {status === 'error' && <div className="error-message"><strong>Error:</strong> {error}</div>}
              {status === 'submitting' && <div className="submitting-message">Procesando...</div>}
              ...
              <div className="form-section">
                ...
                <div className="form-row">
                  <div className="form-group"><label>Región</label>
                    <select name="region" value={formData.region} onChange={handleChange} required style={{padding:'0.8rem', borderRadius:'8px', border:'1px solid #ccc'}}>
                      <option value="">Selecciona una Región</option>
                      {REGIONES_CHILE.map(r => <option key={r.nombre} value={r.nombre}>{r.nombre}</option>)}
                    </select>
                  </div>
                  <div className="form-group"><label>Comuna</label>
                    <select name="ciudad" value={formData.ciudad} onChange={handleChange} required disabled={!formData.region} style={{padding:'0.8rem', borderRadius:'8px', border:'1px solid #ccc'}}>
              ...
              <div className="form-section">
                ...
                <div className="payment-options">
                  <label className="payment-option selected"><input type="radio" checked readOnly /> Webpay / MercadoPago</label>
                </div>
              </div>
              <button type="submit" className="checkout-submit-button" disabled={status === 'submitting'}>{status === 'submitting' ? 'Procesando...' : 'Ir a Pagar'}</button>
            </form>
          </div>
          <div className="checkout-summary-section">
            <div className="checkout-summary">
              ...
              <div className="summary-row total"><span>Total</span><span>CLP${totalPedido.toLocaleString('es-CL')}</span></div>
            </div>
          </div>
        </div>
      )}
